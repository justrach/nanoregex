...
pub fn extractLiteralPrefix(arena: std.mem.Allocator, root: *const ast.Node, min_len: usize) !?[]const u8 {
    var buf: std.ArrayList(u8) = .empty;
    errdefer buf.deinit(arena);
    collectPrefix(root, arena, &buf) catch |err| switch (err) {
...
fn collectPrefix(node: *const ast.Node, arena: std.mem.Allocator, buf: *std.ArrayList(u8)) error{OutOfMemory}!void {
    switch (node.*) {
        .literal => |c| try buf.append(arena, c),
        .concat => |children| for (children) |child| {
            const before = buf.items.len;
            try collectPrefix(child, arena, buf);
            // If this child didn't extend the prefix, stop — the rest of
            // the concat can't contribute a contiguous prefix either.
            if (buf.items.len == before) return;
        },
        .group => |g| try collectPrefix(g.sub, arena, buf),
        .repeat => |r| {
            ...
            if (r.min >= 1) try collectPrefix(r.sub, arena, buf);
